...
def acc_for_threesplits(model, val_loader, args, cls_num_list, many_shot_thr=100, low_shot_thr=20, devide=False):
    model.eval()
    all_preds = []
    all_targets = []
    many_shot = []
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            class_num = len(np.unique(all_targets))
            if args.gpu is not None:
                input = input.cuda(args.gpu, non_blocking=True)
            target = target.cuda(args.gpu, non_blocking=True)
        ...
        all_acc = all_acc/class_num
        return all_acc, np.mean(many_shot), np.mean(medium_shot), np.mean(low_shot)


def get_per_class_acc(val_loader, model, args):
