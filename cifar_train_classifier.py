...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet32',
                    ...
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet32)')
parser.add_argument('--loss_type', default="CBR", type=str, help='loss type')
...
parser.add_argument('--resume', default='checkpoint/cifar100_resnet32_CE_None_exp_0.01_0.001_0/ckpt.best.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    if args.loss_type =='CBR' or args.loss_type =='ABR':
        args.weight_decay = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.__dict__[args.arch](num_classes=num_classes, classifier = True, use_norm= use_norm)
    ...
    # balanced model
    active_layers = [model.linear.weight, model.linear.bias]  # bias对性能影响很小
    ...
    optimizer = torch.optim.SGD([{'params': active_layers}], lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    print('wd:', args.weight_decay)
    # Data loading
    ...
    if args.loss_type == 'CE':
        ...
        criterion = CBLoss(cls_num_list).cuda(args.gpu)
    ...
    elif args.loss_type == 'CBR':
        criterion = CBRLoss(cls_num_list=cls_num_list, weight=per_cls_weights).cuda(args.gpu)
    else:
